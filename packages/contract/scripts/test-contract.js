...
async function main() {
  ...
  try {
    ...
    const billCreatedFilter = payLink.filters.BillCreated(billId);
    const billCreatedEvents = await payLink.queryFilter(billCreatedFilter);
    console.log("  BillCreated events found:", billCreatedEvents.length);
    
    // Get BillPaid events
    const billPaidFilter = payLink.filters.BillPaid(billId);
    const billPaidEvents = await payLink.queryFilter(billPaidFilter);
